...
with st.sidebar:
    ...
    seed=st.number_input("World seed",1,99999999,int(st.session_state.seed),1)
    a,b=st.columns(2)
...
st.markdown("""<div class="tg-kicker">EMERGENT SYSTEMS LAB / TERRARIUM 01</div><div class="tg-title">Tiny Gods</div><div class="tg-sub">A living civilization terrarium. Wanderers become families, settlements become cultures, exceptional lives become legends—and now the most consequential living figures can appear as the world's strange little pantheon.</div><div style="margin-top:10px"><span class="tg-pill">CLICK VILLAGE → ENTER</span><span class="tg-pill">MYTHIC FIGURES = HIGH-IMPACT AGENTS</span><span class="tg-pill">ESC = RETURN</span><span class="tg-pill">SPACE = PAUSE</span></div>""",unsafe_allow_html=True)
html=(ROOT/"terrarium.html").read_text(encoding="utf-8").replace("__TINY_GODS_CONFIG__",json.dumps(config))
patches=["culture_v03.js","spectacle_v04.js","ages_v05.js","legends_v05.js","dynasties_v06.js","card_compat_v08.js","under_glass_v07.js","under_glass_entry_fix_v08.js","identity_v07.js","archaeology_v07.js","construction_v07.js","daily_life_v07.js","role_effects_v07.js","house_life_v07.js","rituals_v07.js","memory_myth_v08.js","myth_transmission_v08.js","pantheon_v09.js","atmosphere_v09.js"]
...
with st.expander("What is actually happening under the glass?"):
    st.markdown("""The pantheon is not a separate scripted cast. Tiny Gods scores living agents from traits and accumulated importance, then gives a few unusually consequential individuals a mythic visual archetype such as Keeper of Roots, Dreams & Memory, Spark & Change, or Watcher of Roads. Under the Glass, residents are tiny humanoid figures with clothing, limbs, role colors and house marks. The newest atmosphere layer adds domain-specific divine effects, gardens, fences, bridges, visible caravan bodies and warm nighttime settlement glow—all derived procedurally from the same live world.""")
st.caption("Self-contained Streamlit showcase · no external assets, accounts, API keys or tracking required.")